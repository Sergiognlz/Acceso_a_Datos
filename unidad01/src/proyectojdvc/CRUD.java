...
public class CRUD {
	...
	public static boolean existeTablaMesa(Connection con) {
		return existeTabla(con, "Mesa");
	}

	public static boolean existeTablaProducto(Connection con) {
		return existeTabla(con, "Producto");
	}

	public static boolean existeTablaFactura(Connection con) {
		return existeTabla(con, "Factura");
	}

	public static boolean existeTablaPedido(Connection con) {
		return existeTabla(con, "Pedido");
	}
	...
	public static boolean insertarPedido(Connection con, int idFactura, int idProducto, int cantidad) {
		if (con == null)
			throw new SinConexionException();
		if (!existeFila(con, "Factura", "idFactura", idFactura)
				|| !existeFila(con, "Producto", "idProducto", idProducto))
			return false;
		if (cantidad < 0) {
	...
	public static List<Integer> obtenerIdsProductoPorDenominacion(Connection con, String denom) throws SQLException {
		...
		try (PreparedStatement ps = con.prepareStatement(sql)) {
			...
			ResultSet rs = ps.executeQuery();
			while (rs.next())
				lista.add(rs.getInt("idProducto"));
		}
		return lista;
	}
	...
	public static void mostrarDatosTabla(Connection con, String tabla) throws SinConexionException {
		...
		try (Statement stmt = con.createStatement(); ResultSet rs = stmt.executeQuery("SELECT * FROM " + tabla)) {
	...
	public static void mostrarProductos(Connection con) throws SQLException {
		...
		try (Statement st = con.createStatement()) {
			...
			while (rs.next()) {
				System.out.printf("%d\t%s\t%.2f\n", rs.getInt("idProducto"), rs.getString("denominacion"),
						rs.getDouble("precio"));
			}
		}
	}
	...
	// modificar filas
	public static boolean modificarFila(Connection con, String tabla, String campoFiltro, Object valorFiltro,
			Map<String, Object> nuevosValores) {
		if (con == null)
			throw new SinConexionException();
		if (nuevosValores.isEmpty())
			return false;
		...
		for (Map.Entry<String, Object> entry : nuevosValores.entrySet()) {
			...
			if ("reserva".equals(campo)) {
				if (!(valor instanceof Number)
						|| (((Number) valor).intValue() != 0 && ((Number) valor).intValue() != 1)) {
		...
		try (PreparedStatement ps = con.prepareStatement(sql.toString())) {
			i = 1;
			for (Object val : nuevosValores.values())
				ps.setObject(i++, val);
	...
	public static void eliminarTablas(Connection con) {
		...
		System.out.println("\n===== ELIMINAR TABLAS =====");
		System.out.println("1. Eliminar una tabla concreta");
		System.out.println("2. Eliminar TODAS las tablas");
		...
		try {
			con.setAutoCommit(false); // Modo transaccional
			...
			if (opcion == 1) {
				...
				System.out.print("Ingrese el nombre EXACTO de la tabla a eliminar: ");
				String tabla = sc.nextLine().trim();

				if (!tabla.equalsIgnoreCase("Mesa") && !tabla.equalsIgnoreCase("Producto")
						&& !tabla.equalsIgnoreCase("Factura") && !tabla.equalsIgnoreCase("Pedido")) {
				...
				boolean tieneDependencias = tabla.equalsIgnoreCase("Mesa") || tabla.equalsIgnoreCase("Producto")
						|| tabla.equalsIgnoreCase("Factura");

				if (tieneDependencias) {
					System.out.println("\n⚠ La tabla " + tabla + " puede tener otras tablas que dependen de ella.");
					System.out.print("¿Desea continuar igualmente? (S/N): ");
					String r2 = sc.nextLine().trim().toUpperCase();
				...
				System.out.println("\nSeguro que desea eliminar la tabla '" + tabla + "'?");
			...
			// CONFIRMACIÓN FINAL (COMMIT/ROLLBACK)
			...
			System.err.println("Error SQL al eliminar tabla(s).");
			e.printStackTrace();
		} finally {
			try {
				con.setAutoCommit(true);
